<template>
    <div class="grouped-notification  pl-21 flex" @click="showGrouped(true)" :class="{todayColor : today}">
        <div class="notification__item flex-between flex-align-center">
            <picture-reusable
                    ...
                    text-layout="bottom">
                <template #title>
                    <span > {{author.username}} <lang-string  class="event-span lowercase" :title="'participated_in'" /> {{items.length}} <lang-string class=" lowercase event-span" :title="'events'" /></span>
                </template>
