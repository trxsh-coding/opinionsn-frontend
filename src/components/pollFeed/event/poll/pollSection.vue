	...
	<div class="poll-block pb-10">
		<div class="middle-block">
			...
			<div class="description-block wb" v-if="!hiddenText">
				<span class="poll-description">{{normDescription}}</span>
				<span class="more" @click="expandText" v-if="!showMore">ещё</span>
			</div>
			<div class="description-block wb" v-if="hiddenText">
			...
			<div
				class="picture-block"
				v-if="poll.picture"
				:style="{ 'background-image': 'url(' + poll.picture + ')' } "
			></div>
		</div>
		<div class="options">
			<div
				class="options-section"
				v-for="(option, option_index) in senitizedOptions"
				:key="option_index"
			>
				<el-button
					v-if="user && user.authorities === 'ADMIN'  "
					@click="setRightOption(option.id, item.id)"
				>✔</el-button>
	...
	export default {
		...
		computed: {
			...
			normDescription: function() {
				...
				if (text.length >= 300) {
					return (
						text.substring(0, text.substring(0, 200).lastIndexOf(" ")) +
						"... "
					);
				} else {
					this.showMore = true;
			...
			senitizedOptions: function() {
				...
				sanOptions.splice(
					expanded ? options.length : optionsLimit,
					options.length
				);
				return sanOptions;
			}
		...
		methods: {
			...
			setRightOption(selected_variable, poll_id) {
				if (this.poll.type_of_poll >= 2) {
					this.$store.dispatch(`pollFeed/setBlockchainRightOption`, {
						data: { selected_variable, poll_id }
					});
				} else {
					this.$store.dispatch(`pollFeed/setRightOption`, {
						data: { selected_variable, poll_id }
					});
				}
			},
				...
				if (this.expanded) {
					if (!this.item.voted) {
						if (this.$route.name === "pollFeed") {
							this.$store.dispatch(`pollFeed/createVote`, {
								data: { selected_variable, poll_id }
							});
						} else {
							this.$store.dispatch(`singlePoll/createVote`, {
								data: { selected_variable, poll_id }
							});
						}
					}
